...
@Module({
  imports: [PrismaModule, JwtModule.register({ global: true, secret: jwtSecret ?? 'development-only-secret', signOptions: { expiresIn: '8h' } }), AuthModule, InvitationsModule, CareModule, UsersModule],
  controllers: [HealthController], providers: [{ provide: APP_GUARD, useClass: JwtAuthGuard }],
})
